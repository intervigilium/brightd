...
int getBrightness() { /*{{{*/
  ...
  if (!brightness) {
    error("Failed to open /sys/class/backlight/*/actual_brightness. Do you "
          "have the "
...
void signalHandlerQuit(int signal) { /*{{{*/
  ...
  if (bfifoFd != 0) {
    ...
    close(bfifoFd);
    if (unlink(b_fifo) != 0) {
      info("Failed to remove FIFO - this does only work when you didn't drop "
           "privileges");
    }
  }
...
{
#ifdef X11
  printf("brightd %s\nA X11-daemon for iBook-like brightness "
         "management\nCopyright (c) 2006-2008, Phillip Berndt\n\nOptions:\n",
         RELEASE);
#else
  printf("brightd %s\nA daemon for iBook-like brightness management\nCopyright "
         "(c) 2006-2008, Phillip Berndt\n\nOptions:\n",
         RELEASE);
#endif
  printf(" -v   Be verbose\n");
  printf(" -d   Daemonize\n");
  ...
  printf(" -w n Wait n seconds before reducing brightness (Defaults "
         "to 3)\n");
  printf(" -b n The brightness setting for the dark screen (Defaults "
         "zu 0)\n");
  printf(" -f   Reduce brightness even if on the highest brightness "
         "level\n");
  printf("      Specify twice to also do so when on AC\n");
  printf(" -e n  Filter event sources using regexp n (on "
         "/dev/input/by-path\n");
...
#endif
  printf(" -r n Create a FIFO, into which acpid may write the new "
         "level when the user\n");
...
int main(int argc, char *argv[]) { /*{{{*/
  ...
  *pidfile = 0;
  ...
  if (inotifyFdEvents != -1) {
    if (inotify_add_watch(inotifyFdEvents, "/dev/input/by-path/", IN_CREATE) ==
        -1) {
  ...
  while (1) {
    ...
    if (inotifyFdEvents != -1 && FD_ISSET(inotifyFdEvents, &rfds)) {
      ...
      if (inotifyEvent->len > 0 &&
          strstr(inotifyEvent->name, "event") != NULL) {
